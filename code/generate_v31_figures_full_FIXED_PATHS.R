...
find_repo_root <- function(start = getwd()) {
  ...
  repeat {
    ...
    if (identical(parent, current)) {
      stop("Could not find repository root containing data/processed and results/tables.")
    }
    current <- parent
  }
}
...
output_dir <- file.path(base_dir, "reproduction", "figures_v31")
...
candidate_input_dirs <- unique(candidate_input_dirs[dir.exists(candidate_input_dirs)])

if (length(candidate_input_dirs) == 0) {
  stop("None of the candidate input folders exist. Check the base path: ", base_dir)
}
...
find_input_file <- function(filename) {
  # First search directly in each candidate folder.
  direct_hits <- file.path(candidate_input_dirs, filename)
  direct_hits <- direct_hits[file.exists(direct_hits)]
  if (length(direct_hits) > 0) return(normalizePath(direct_hits[[1]], winslash = "/", mustWork = TRUE))

  # Then search one level recursively. This catches files inside "final outputs" even
  # if the script is launched from another working directory.
  recursive_hits <- unlist(lapply(candidate_input_dirs, function(d) {
    list.files(d, pattern = paste0("^", gsub("\\.", "\\\\.", filename), "$"),
...
input_paths <- purrr::map_chr(files, find_input_file)
missing_files <- names(input_paths)[is.na(input_paths)]

if (length(missing_files) > 0) {
  searched <- paste0("  - ", candidate_input_dirs, collapse = "\n")
  missing_msg <- paste0("  - ", unname(unlist(files[missing_files])), collapse = "\n")
  stop(
    "Missing required v31 input files.\n\n",
    "Searched these folders:\n", searched, "\n\n",
    "Missing files:\n", missing_msg, "\n\n",
    "Expected inputs live in data/processed and results/tables in this repository."
  )
}

input_file_manifest <- tibble::tibble(
  key = names(input_paths),
  filename = unname(unlist(files)),
  path = unname(input_paths)
)

readr::write_csv(input_file_manifest, file.path(figure_data_dir, "00_input_file_manifest.csv"))
message("Found all required input files:")
print(input_file_manifest, n = Inf)

read_v31 <- function(key) {
  readr::read_csv(input_paths[[key]], show_col_types = FALSE)
}

# Keep this for README compatibility. It records the folder where most files were found.
input_dir <- dirname(input_paths[[1]])
